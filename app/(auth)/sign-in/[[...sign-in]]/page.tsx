...
import { Sparkles, Zap, Heart, Award, Brain } from "lucide-react"; 
...
export default function Page() {
  ...
  // To ensure animations only run on the client-side
  useEffect(() => {
    setIsMounted(true);
  }, []);
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-slate-50 via-white to-red-50 dark:from-slate-900 dark:via-slate-800 dark:to-red-900 p-4">
      ...
      <div className="absolute inset-0 z-0">
        {/* Pulsing Gradient Spheres */}
        <motion.div
          initial={{ opacity: 0, scale: 0.5, x: "-50%", y: "-50%" }}
          animate={{ opacity: 1, scale: [0.5, 1.2, 0.8], x: "-50%", y: "-50%", rotate: 360 }}
          transition={{ duration: 8, repeat: Infinity, repeatType: "reverse", ease: "linear" }}
          className="absolute top-1/2 left-1/2 h-96 w-96 rounded-full bg-red-400/30 blur-3xl"
        />
        <motion.div
          initial={{ opacity: 0, scale: 0.5, x: "50%", y: "50%" }}
          animate={{ opacity: 1, scale: [0.5, 1.2, 0.8], x: "50%", y: "50%", rotate: -360 }}
          transition={{ duration: 10, delay: 2, repeat: Infinity, repeatType: "reverse", ease: "linear" }}
          className="absolute bottom-1/4 right-1/4 h-80 w-80 rounded-full bg-yellow-400/30 blur-3xl"
        />
        ...
        {[...Array(40)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute h-[2px] w-[2px] rounded-full bg-orange-400/50"
            initial={{
      ...
      <motion.div
        initial={{ opacity: 0, scale: 0.7, y: 100, rotateX: 90 }}
      ...
      >
        {/* Animated Brain Icon at the top */}
        <div className="flex items-center justify-center gap-4 mb-8">
          <motion.div
            initial={{ scale: 0, rotate: -180 }}
            animate={{ scale: 1, rotate: 0 }}
            transition={{ duration: 1.2, type: "spring", bounce: 0.5, delay: 0.5 }}
            className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-yellow-500 via-orange-600 to-red-600 shadow-xl"
          >
            <Brain className="w-8 h-8 text-white" />
            <motion.div
              animate={{ scale: [1, 1.2, 1] }}
              transition={{ duration: 2, repeat: Infinity }}
              className="absolute w-16 h-16 rounded-2xl border-4 border-yellow-400/50"
            />
          </motion.div>
        </div>
        ...
        >
          <motion.div variants={itemVariants} className="flex items-center gap-1">
            <Heart className="h-4 w-4 text-red-500" /> Secure & Easy
          </motion.div>
          <motion.div variants={itemVariants} className="flex items-center gap-1">
            <Award className="h-4 w-4 text-yellow-500" /> Trusted by 100K+
          </motion.div>
        </motion.div>
      </motion.div>
    </div>
